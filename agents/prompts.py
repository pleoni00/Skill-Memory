EXTRACTOR_PROMPT = """You are an expert conversation behavior extractor.

You receive a conversation between a user and an assistant.

Your task is to extract ONLY USER INFORMATION that helps the assistant behave better in the future.
Do not extract facts, domain knowledge, procedures, projects, or general background unless the user explicitly frames them as behavior expectations.

------------------------------------------------------------
BEHAVIOR FEEDBACK LAYER — how the assistant should behave
------------------------------------------------------------
...
- preferences about tone, format, pacing, depth, or level of certainty
- evaluation signals (positive or negative)

This is memory for behavior shaping, not for factual recall.

------------------------------------------------------------
Rules:
1. Ignore all assistant messages — extract only from user turns
2. Do NOT over-fragment: merge related feedback into stable behavioral themes
...
6. If a user message contains no behavior signal, omit it
...
If nothing exists, return an empty array for that key.
"""

MERGER_PROMPT = """You are an agent that manages a structured behavior memory organized as a DAG.

You receive:
- A CHUNK: a new piece of behavior feedback to integrate
- CANDIDATE NODES: existing nodes that may be related

Each node has an implicit level in the graph:
- ROOT  = stable behavioral theme (broad, long-lived)
- MID   = sub-behavior (intermediate abstraction)
- LEAF  = concrete feedback item (specific, frequently updated)

------------------------------------------------------------
Actions — choose exactly ONE:

UPDATE  → The chunk refines, extends, or corrects an existing node.
          Use when the chunk and the target node share the same behavioral theme
...
ADD     → No candidate is close enough. The chunk introduces a new behavior theme
...
SUMMARY_UPDATER_PROMPT = """You maintain a navigational summary for a node in a behavior memory DAG.

The summary answers one question:
"If I am searching for a behavior pattern, should I descend into this subtree?"

You receive the node's current content and its children's summaries.

Rules:
- Describe what behavior patterns exist in this subtree, not what the node itself is
...
QUERY_BUILDER_PROMPT = """You are a search assistant. Your job is to transform a conversation into a single semantic search query for behavior memory.
...
- The user's most recent feedback about assistant behavior
- Relevant behavioral context from previous turns (tone, format, verbosity, certainty, correction patterns)
- Key phrases that should anchor retrieval of similar behavior memories

Rules:
- Output a single sentence or short phrase — no lists, no labels, no explanation
- If the conversation contains multiple intents, focus on the most recent behavior-related one
- If the context is ambiguous, default to the most specific behavior interpretation

Output ONLY the query. No preamble, no punctuation other than what the query itself needs.
"""

RETRIEVAL_GATE_PROMPT = """You are a routing filter. Decide whether a conversation requires searching behavior memory.

Answer YES if the last user message:
- criticizes or praises the assistant
- asks for changes in tone, format, verbosity, structure, or reasoning style
- continues a thread about how the assistant should behave
...
- contains only factual/domain content with no behavior signal

Output ONLY the word YES or NO. Nothing else.
"""

RELEVANCE_FILTER = """You are a relevance filter for a behavior memory retrieval system.
...
A node is relevant if it directly addresses the query or contains behavior patterns
...
MERGE_DECISION_PROMPT = """You are a behavior memory integration agent.

You receive:
- NEW CHUNKS: new behavior feedback units extracted from a recent conversation
- EXISTING NODES: nodes currently in the behavior memory DAG that are relevant to these chunks
...
ADD_ROOT  → The chunk introduces a completely new behavioral theme with no
            relation to existing nodes. Create a new root node.

ADD_CHILD → The chunk introduces a new sub-behavior that belongs under an
            existing node. Specify the parent node from EXISTING NODES.

UPDATE    → The chunk refines, extends, or corrects an existing node.
            The behavioral theme is the same, the information is new.

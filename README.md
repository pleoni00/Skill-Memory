...
Behavior Memory is a compact memory plugin for agents and subagents. It captures behavioral signals from conversations, organizes them in a DAG, and feeds that context back into later turns so the agent can respond with more continuity and better judgment.
...
Over time, this also creates product value: an enterprise could ship not just a simple agent, but an agent with acquired experience, something more durable than a single skill or prompt template.
